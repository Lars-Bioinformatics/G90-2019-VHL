...
configfile: "/work/sduvarcall/G90-2019-VHL-case-projekt/samples_matched_pairwise.yaml"
...
output_somatic = "mutect2_somatic_bamOut/"
...
rule Mutect2_matched:
    input:
        ...
        tumor=lambda wildcards: expand("bam/{tumor}_recal.bam", tumor=config[wildcards.sample]["tumor"]),
        pon=pon_location+"pon.vcf.gz",
        intervals=target_regions+"{interval}.interval_list"
    output:
    ...
    params:
        tumor=lambda wildcards: expand("-I bam/{tumor}_recal.bam", tumor=config[wildcards.sample]["tumor"]),
...
rule merge_mutect2_bam:
  input:
        bam_subfile=expand(output_somatic+"bam_split/{{sample}}__{interval}_mutect2.bam", interval=INTERVALS)
  output:
        bam=output_somatic+"realigned_bam/{sample}.bam"
  params:
        bam_subfile=expand("-I "+output_somatic+"bam_split/{{sample}}__{interval}_mutect2.bam", interval=INTERVALS)
  shell:
        """
...
rule JoinContaminationTables:
    input:
        cont_tables=lambda wildcards: expand(output_somatic+"contamination/{tumor}_vs_{normal}_contamination.table", 
                    normal=config[wildcards.sample]["normal"], tumor=config[wildcards.sample]["tumor"])
    output:
